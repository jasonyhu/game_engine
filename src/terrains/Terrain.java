...
//	private static final int VERTEX_COUNT = 128;
	...
	public float getHeightOfTerrain(float worldX, float worldZ) {
		...
		float answer;
		if (xCoord <= (1 - zCoord)) {
			answer = GaMath.baryCentric(new Vector3f(0, heights[gridX][gridZ], 0),
					new Vector3f(1, heights[gridX + 1][gridZ], 0), new Vector3f(0, heights[gridX][gridZ + 1], 1),
					new Vector2f(xCoord, zCoord));
		} else {
			answer = GaMath.baryCentric(new Vector3f(1, heights[gridX + 1][gridZ], 0),
	...
	public static int getTerrainXTile(float x, int offset) {
		int gridX = (int) Math.floor(x / SIZE);
		return gridX + offset;
	}
	
	public static int getTerrainZTile(float z, int offset) {
